...
def detect_face_rotate(img_file, base_dir, out_dir = 'out', debug_mode = False):
    ...
    for deg in range(-5, 6, 5):
        ...
        for (x, y, w, h) in faces:
            ...
            if debug_mode:
                out_file = '%s/face_%s_%s_%s.jpg' % (img_dir, result['face_id'], deg, center_org)
                print('face candidate', out_file)
    ...
    if debug_mode:
        for cand in filtered:
            ...
            print('filtered candidate', out_file)
    ...
    for item in filtered:
        ...
        if debug_mode:
            ...
            print('mouth candidates', out_file)
            cv2.imwrite(out_file, img)
            #show(img)
...
def crop_color_face(item, img, base_dir, out_dir, fn):
    ...
    if debug_mode:
        out_file = '%s/7_color_deg_%s.jpg' % (base_dir, deg)
        cv2.imwrite(out_file, rotated)
...
if __name__ == "__main__":
    ...
    if len(param) <= 3:
        ...
        if param[3] == 'debug':
            ...
            debug_mode = False
    detect_face_rotate(param[1], '.', param[2], debug_mode)
